...
function PrivateBlogPost(props) {
  return(
    <div className="private-posts">
      <h2>{props.title}</h2>
      <p class="blog-post.meta">By {props.author} on {props.date}</p>
      <p>The content of this post is private </p>
    </div>
  )
}

function PublicBlogPosts(props) {
  return (
...
function BlogList() {
  return <div className="blog-list"><ul>
      {blogPosts.map(post => {
          if (post.isPrivate) {
            return (<PrivateBlogPost title={post.title} author={post.author} date={post.date} />)
          } else {
            return (<PublicBlogPosts title={post.title} author={post.author} date={post.date} content={post.content} />)
          }
})}
...
function Header(props) {
  return (
    ...
    <h1 id="header h1">{props.name}</h1>
    <p id="header p">{props.tagline}</p>
    </header>
    );
}
...
function Footer(prop) {
  return (
    <footer className="footer">
      <p> &#169; {prop.year} My Blog. All rights reserved.</p>
    </footer>
  )
};
...
function App() {
  ...
  return (
    <div>
      <Header name="My Blog" tagline="A blog about everything"/>
      <BlogList />
      <Footer year="2024"/>
    </div>
